...
@pytest.fixture()
async def get_user_client(vsa_engine) -> AsyncClient:
    reset_session_state()
...
@pytest.mark.asyncio
async def test_get_user_without_jwt_returns_unauthorized(
    get_user_client: AsyncClient,
) -> None:
    response = await get_user_client.get("/api/v1/users/1")

    assert response.status_code == 401


@pytest.mark.asyncio
async def test_get_user_with_jwt_returns_user(
    get_user_client: AsyncClient,
...
@pytest.mark.asyncio
async def test_get_user_not_found_returns_users_not_found(
    get_user_client: AsyncClient,
    vsa_session: AsyncSession,
    password_hasher: Argon2PasswordHasher,
) -> None:
    ...
    response = await get_user_client.get("/api/v1/users/9999", headers=headers)
