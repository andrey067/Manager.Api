...
@pytest.fixture()
async def get_user_by_email_client(vsa_engine) -> AsyncClient:
    reset_session_state()
...
@pytest.mark.asyncio
async def test_get_user_by_email_without_jwt_returns_unauthorized(
    get_user_by_email_client: AsyncClient,
) -> None:
    response = await get_user_by_email_client.get(
...
@pytest.mark.asyncio
async def test_get_user_by_email_with_jwt_returns_user(
    get_user_by_email_client: AsyncClient,
...
@pytest.mark.asyncio
async def test_get_user_by_email_not_found_returns_users_not_found(
    get_user_by_email_client: AsyncClient,
    vsa_session: AsyncSession,
    password_hasher: Argon2PasswordHasher,
) -> None:
