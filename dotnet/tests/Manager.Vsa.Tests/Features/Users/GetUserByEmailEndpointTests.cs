...
public class GetUserByEmailEndpointTests(CreateUserWebApplicationFactory factory)
{
    private readonly HttpClient _client = factory.CreateClient();

    [Fact]
    public async Task GetUserByEmail_WithoutJwt_ReturnsUnauthorized()
    {
        var response = await _client.GetAsync("/api/v1/users/by-email?email=admin@example.com");
    ...
    private async Task<HttpResponseMessage> GetUserByEmailAsync(string token, string email)
    {
        var request = new HttpRequestMessage(HttpMethod.Get, $"/api/v1/users/by-email?email={Uri.EscapeDataString(email)}");
